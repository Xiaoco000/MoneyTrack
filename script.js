  ...
  onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      if (t.userId === userId) {
        ...
        li.innerHTML = `
          <div>
            <b>${t.title}</b><br><small style="color:#888">${t.category}</small>
          </div>
          <div>
            <span style="color:${t.type === 'income' ? 'green' : 'red'}; font-weight:600;">
              ${t.type === 'income' ? '+' : '-'} Rp ${t.amount.toLocaleString('id-ID')}
            </span>
            <button onclick="deleteTrans('${docSnap.id}')" style="border:none;background:none;color:red;cursor:pointer;margin-left:8px;">✕</button>
          </div>
        `;
...
function setupSearchUser() {
  ...
  searchInput.addEventListener('input', async () => {
    ...
    if (found) {
      ...
      searchResults.innerHTML = '<div style="padding:10px; font-size:0.8rem; color:#888; text-align:center;">User tidak ditemukan</div>';
      searchResults.classList.remove('hidden');
    }
  });

  // Sembunyikan hasil pencarian jika mengklik di luar area
...
function loadChatData() {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      div.innerHTML = `
        <span style="font-size:0.75rem; color:#1a73e8; font-weight:700;">@${c.sender}</span>
