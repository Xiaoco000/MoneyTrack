...
const sendOtpBtn = document.getElementById('sendOtpBtn'); // Digunakan sebagai tombol Google Login
const authMessage = document.getElementById('authMessage');
const userDisplay = document.getElementById('userDisplay');
const logoutBtn = document.getElementById('logoutBtn');

// 1. Login dengan Google (Bebas Kuota Limit & Banyak Orang)
sendOtpBtn.addEventListener('click', async () => {
  try {
    authMessage.innerText = '⏳ Sedang mengarahkan ke Google...';
    await signInWithPopup(auth, googleProvider);
  } catch (err) {
    authMessage.innerText = '❌ Gagal Login: ' + err.message;
  }
});

// 2. Monitor User Login & Sync Database Cloud
...
// 3. Sinkronisasi Data Firestore Cloud Per User
