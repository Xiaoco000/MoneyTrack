...
import { getFirestore, collection, addDoc, onSnapshot, deleteDoc, doc, setDoc, updateDoc, query, orderBy, getDocs, getDoc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
  ...
  onSnapshot(query(collection(db, "transactions")), (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      if (t.userId === uid) {
        ...
        li.style.cssText = "background:white; padding:12px; border-radius:18px; border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;";
        li.innerHTML = `
...
function loadFeedData() {
  onSnapshot(query(collection(db, "posts"), orderBy("createdAt", "desc")), (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      let imgTag = p.image ? `<img src="${p.image}" style="width:100%; border-radius:16px; margin-top:8px;">` : '';
  ...
  onSnapshot(collection(db, "groups"), (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      if ((g.members || []).includes(currentUser.uid)) {
        let div = document.createElement('div');
        div.style.cssText = "background:white; padding:14px; border-radius:20px; border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;";
        div.innerHTML = `
          <div>
            <b>👥 ${g.name}</b>
